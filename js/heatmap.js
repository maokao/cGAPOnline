...
var ycov_cellHeight = 8;    //equal cellHeight
...
var rgbcube_y = 1;
var rgbcube_z = 1;
var fromExample = true;
var nodeMode = 0;   //0:albxx, 1:albcat, 2:all
var lineMode = 0;
var textMode = 0;
var scene;
var firstInitScene = true;
...
function heatmap_display(url, heatmapId, paletteName, delimiter) {
    ...
    function zoom() {
        svg.attr('transform', 'translate(' + (viewerPosLeft+d3.event.transform.x) + ',' + (viewerPosTop+d3.event.transform.y+cellWidth*col_number) + ') scale(' + d3.event.transform.k + ')');
    }
    ...
    d3.text(url).then(function(textString) {
        ...
        //albcat_Y = -10-hml_maxcat*xcov_cellHeight;

        if(xc>0)
        {
            row_number = row_number - xc;
            //xc_Y = -10-xc*xcov_cellHeight-albcat_Y; 
        ...
        if(xd>0)
        {
            ...
            //xd_Y = xc_Y -10-xd*xcov_cellHeight-albcat_Y;
...
/*
        ...
        {
            if(yd>0)
                albxx_X = -3-albxx_cellWidth+yd_X;
            else
                albxx_X = -3-albxx_cellWidth+yc_X;
        }
        else
        {
            if(yd>0)
                ...
                albxx_X = -3-albxx_cellWidth;
        }*/

        console.log("row_number: "+row_number);
        console.log("col_number: "+col_number);
        var size = String(row_number)+" x "+String(col_number);
        if(fromExample == true)
        {
            $("#example_size").text("("+size+")");
            $("#uploadFile_size").text("");
        }
        else
        {
            $("#uploadFile_size").text("("+size+")");
            $("#example_size").text("");
        }
        ...
        for( i=0 ;i< row_number; i++)
        {
            ...
            {
                if(dataset[i+xCov][dataset.columns[j+yCov+yN]]=="NA")
                    tempdata.push(missing_value);
                else
        ...
        //for cGAP
        hml_acat = new Array(col_number);
        hml_amaxcat = new Array(col_number);
        hml_amincat = new Array(col_number);
        hml_atotalcat = new Array(col_number);
        hml_amaxcat_temp = new Array(col_number);
        ncat = 0;
        ...
        for(j=0;j<col_number;j++)
         ...
         for(j=0;j<col_number;j++){
             for(i=0;i<row_number;i++){
                 if(data[i][j]>hml_amaxcat[j]&&data[i][j]!=missing_value) 
                    hml_amaxcat[j]=data[i][j];
                 if(data[i][j]<hml_amincat[j]&&data[i][j]!=missing_value) 
                    hml_amincat[j]=data[i][j];
                 //pt++;
             }
         }
...
//console.log("hml_atotalcat["+j+"]: "+hml_atotalcat[j]+","+hml_amincat[j]+","+hml_amaxcat[j]);
         }
         for(j=0;j<col_number;j++)
         {
             if(hml_amincat[j]>1)
         ...
         for(j=0;j<col_number;j++){
            //console.log("hml_acat["+j+"]: "+hml_acat[j]);
             ncat+=hml_acat[j];
         }
        ...
        if(yc>0)
        {
            yc_X = -10-yc*ycov_cellWidth+albxx_X; 
            //console.log("yc_X:"+yc_X);  
        }
            
        if(yd>0)
        {
            yd_X = yc_X -10-yd*ycov_cellWidth+albxx_X;
            //console.log("yd_X:"+yd_X);  
        }
/*
        if(yc>0)
...
*/
        ...
        var y_ratio = Math.ceil((viewerHeight-250)/(col_number+row_number+hml_maxcat));
        ...
        //})
        .on("dblclick.zoom", null)  //取消d3中 doubleclick會放大的功能
            .append("g")
            .attr("id", "gap")
            .attr("transform", "translate(" + viewerPosLeft + "," + (viewerPosTop+cellWidth*col_number) + ")");
        ...
        var colLabels = svg.append("g")
            ...
            .attr("transform", function(d, i) {
                ...
                {
                    if(xd>0)
                        return "translate(" + cellWidth / 2 + ", "+(-3+xd_Y+albcat_Y)+") rotate(-90) rotate(45, 0, " + (i * cellWidth) + ")";
                    else
                        return "translate(" + cellWidth / 2 + ", "+(-3+xc_Y+albcat_Y)+") rotate(-90) rotate(45, 0, " + (i * cellWidth) + ")";
                }
                else
                {
                    if(xd>0)
                        ...
                        return "translate(" + cellWidth / 2 + ", "+(-3+albcat_Y)+") rotate(-90) rotate(45, 0, " + (i * cellWidth) + ")";
                }
        ...
        //setupHeatmap2(data,"mv",0,0,99, heatmapId, getColorID(rdPalette));
        ...
        //run homals
        runHomals(data,row_number,col_number);

        //draw raw data matrix plot
        setupHeatmap2(data,"mv",0,0,99, heatmapId, getColorID(rdPalette));
        ...
        {   
            tempdata = [];
            tempdata.push(i); 
            tmpdata.push(tempdata);
        }
        setupHeatmap_albxx(tmpdata,"mv20",albxx_X,0, heatmapId);

        //draw albcat plot
        setupHeatmap_albcat(cat_list_2,"mv21",0 ,albcat_Y, heatmapId);
        ...
        var colorID = d3.interpolateSpectral;
            ...
            if(isColProxfirst)
            {
                var colProxY = -10-col_number*cellWidth+albcat_Y;
                if(xc>0)
                {
                    if(xd>0)
...
*/
        ...
        d3.select("#optionDataMap").on("change", function() {
           ...
           if(optionTargetDataMap == "rawdata")
           {
                ...
                if(rdPalette == "cGAP")
                {
                    $("#palette option[value='cGAP']").attr('selected','selected');
                    $("#isColorReverse").attr('disabled','disabled');
                    $("#displaycondition").attr('disabled','disabled');    
                    $('#restricted_name').css('display', 'none');
                    $('#restricted_display').css('display', 'none'); 
                }
                else
                {
                    $("#isColorReverse").removeAttr('disabled');
                    $("#displaycondition").removeAttr('disabled');
                    $('#restricted_name').css('display', '');
                    $('#restricted_display').css('display', '');
                }
           ...
           {
                $("#palette option[value='cGAP']").attr('disabled','disabled');
                $("#isColorReverse").removeAttr('disabled');
                $("#displaycondition").removeAttr('disabled');
                $('#restricted_name').css('display', '');
                $('#restricted_display').css('display', '');
        ...
        d3.select("#palette")
            .on("mouseup", function() {
                ...
                if(optionTargetDataMap == "rawdata")
                {
                    if(d3.select("#palette").property("value") == "cGAP")
                    {
